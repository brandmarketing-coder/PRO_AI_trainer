...
- **健檢工具**：`node scripts/check-knowledge.js` — 檢查每個檔案讀得到、切塊正常，並用代表性業務問題做檢索煙霧測試（含口語問法）。新增／大改知識檔後、或懷疑 AI 答錯來源時跑一次，全部 PASS 才算穩。
- **資料來源標註**：知識問答的每個回答下方會標「資料來源：某某檔」，答案可追溯；業務發現答錯時，主管能直接找到該知識檔修正。
- **口語同義詞**：業務口語（「太貴了」「頭很油」「白頭髮」）與知識庫正式詞彙（價格、控油、麥拉寧）的落差由 `knowledge.js` 的 `SYNONYMS` 表轉換；發現新的「問了查不到」情況，在該表加一行即可。
- **擴展性**：知識庫開機載入記憶體、檢索為線性掃描，實測 426 段約 3ms／次；到數千段（約現在 10 倍）都毫無壓力。檔名用數字前綴（`13_`、`14_`…）控制優先順序；未列入優先清單的新檔會自動排在後面。
