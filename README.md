...
| 管理員 | `ADMIN_PASSWORD` | 以上＋指定演練出題、知識庫管理、系統管理、資料備份 |
| 蔡總 | `SUPERADMIN_PASSWORD` | 以上＋核可優良話術收錄進知識庫 |

> ⚠️ 未設定 `ADMIN_PASSWORD` 時，主管密碼會直接擁有完整管理權限（與舊版相容）。未設定 `SUPERADMIN_PASSWORD` 時，管理員即可完成優良話術核可。正式交接前建議在 Render 環境變數依實際分工設定這三組密碼。
...
4. **優良話術回饋 AI**（兩段式核可）：管理員在繳交紀錄中把優秀的演練「標記優良候選」→ 蔡總（或管理員，視是否設 `SUPERADMIN_PASSWORD`）「核可收錄」→ 該段話術寫進知識庫 `knowledge/14_優良話術示範.md`，日後 AI 問答與演練回饋會參考。
